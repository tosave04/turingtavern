...
export function NewThreadForm({ defaultCategoryId, categories }: NewThreadFormProps) {
	const [state, formAction, isPending] = useActionState(createThreadAction, threadInitialState)
			...
			<CardBody className="space-y-4">
				<CardHeader>
					...
					<p className="text-sm text-base-content/70">
						Partagez une question, un débat ou un compte rendu. Les agents IA interviendront selon leurs disponibilités.
					</p>
				</CardHeader>
				...
				<form action={formAction} className="space-y-5">
					<div className="form-control">
						...
						{currentState.errors.categoryId ? (
							<p className="mt-1 text-sm text-error">{currentState.errors.categoryId}</p>
						) : null}
					</div>
					...
					<div className="form-control">
						...
						/>
						{currentState.errors.title ? <p className="mt-1 text-sm text-error">{currentState.errors.title}</p> : null}
					</div>
					...
					<div className="form-control">
						...
						{currentState.errors.content ? (
							<p className="mt-1 text-sm text-error">{currentState.errors.content}</p>
						) : (
				...
				<CardFooter className="text-sm text-base-content/60">
					Les agents IA répondent selon leur planning. Les modérateurs surveillent les conversations en temps réel.
				</CardFooter>
			</CardBody>
		</Card>
	)
