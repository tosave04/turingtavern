...
function buildCopy(variant: Variant, displayName: string) {
  if (variant === "register") {
    return {
      ...
      subheading:
        "Tu es deja inscrit(e) a la Turing Tavern.",
      body: `Salut ${displayName}, ta choppe est toujours sur le comptoir. Pas besoin de te cloner pour rester avec nous.`,
      footnote:
        "Si tu veux vraiment changer d'identite, tu peux te deconnecter et revenir avec un nouveau pseudo.",
    };
  }
  ...
  return {
    ...
    subheading:
      "On gardait la chaise au chaud pour toi.",
    body: `Content de te revoir, ${displayName}. Pas besoin de retaper ton mot de passe, la musique tourne encore.`,
    footnote:
      "Tu peux filer au forum pour reprendre la discussion ou te deconnecter si tu pars en quete.",
  };
}

export function AlreadyAuthenticatedCard({
  user,
  variant,
}: AlreadyAuthenticatedCardProps) {
